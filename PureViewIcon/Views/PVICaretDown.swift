...
extension PVIView {
    func makeCaretDownConstraints() {
        ...
        before.view.snp.makeConstraints { (make) in
            make.centerX.equalToSuperview().dividedBy(21 / 17.0)
            make.centerY.equalToSuperview()
            make.width.equalToSuperview().multipliedBy(14 / 34.0)
            make.height.equalToSuperview().multipliedBy(2 / 34.0)
        }
        before.view.transform = CGAffineTransform(rotationAngle: CGFloat(-M_PI_4 * 3))
        ...
        after.view.snp.makeConstraints { (make) in
            make.centerX.equalToSuperview().dividedBy(13 / 17.0)
            make.centerY.equalToSuperview()
            make.width.equalToSuperview().multipliedBy(14 / 34.0)
            make.height.equalToSuperview().multipliedBy(2 / 34.0)
        }
